...
- **Язык конспекта**: писать связный текст, заголовки, таблицы, альтернативный
  текст изображений и подписи схем на естественном русском языке. В обычной
  фразе использовать «языковая модель», «агент», «инструмент», «вызов
  инструмента», «инструкция» и «контекст», а не `LLM`, `Agent`, `tool`,
  ...
  продуктов. При необходимости после русского объяснения указывать точное имя
  в обратных кавычках: «ошибка отсутствия совместимого маршрута
